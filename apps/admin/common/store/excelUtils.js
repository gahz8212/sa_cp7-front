...
exports.detectDataArea = exports.filterRows = exports.getActiveRowBounds = exports.getActiveColumnBounds = exports.classifyRow = exports.isApprovalLineRow = exports.isGeneralNoiseRow = exports.isStringOnlyRow = exports.getSignature = exports.getCellType = exports.rowToValues = void 0;
// ── 1. 셀 및 행 기본 유틸리티 ───────────────────────────────────────────
...
const getSignature = (row) => {
    return (0, exports.rowToValues)(row).map((v) => (0, exports.getCellType)(v));
};
 ...
 */
const isGeneralNoiseRow = (row, totalCols) => {
    ...
    if (nonEmptyCount <= Math.max(2, Math.floor(totalCols * 0.25)))
        return true;
    return false;
};
exports.isGeneralNoiseRow = isGeneralNoiseRow;
/**
 * 구조 기반 결제선 행 판별 (키워드 비의존성)
 */
const isApprovalLineRow = (row, idx, rows, totalCols) => {
    const values = (0, exports.rowToValues)(row).map((v) => String(v ?? '').trim());
    ...
    if (density <= 0.65) {
        ...
        if (rightHalfCount >= 2) {
            ...
            if (nextRow) {
                const nextValues = (0, exports.rowToValues)(nextRow).map((v) => String(v ?? '').trim());
...
exports.isApprovalLineRow = isApprovalLineRow;
/**
 * 특정 행의 카테고리를 엄격하게 분류하는 판별기
 */
const classifyRow = (row, idx, rows, totalCols) => {
    const values = (0, exports.rowToValues)(row).map((v) => String(v ?? '').trim());
    const nonEmptyCount = values.filter((v) => v !== '').length;
    const sig = (0, exports.getSignature)(row);
    if (nonEmptyCount === 0) {
        return 'EMPTY';
    }
    if ((0, exports.isApprovalLineRow)(row, idx, rows, totalCols)) {
        return 'APPROVAL';
    }
    if ((0, exports.isGeneralNoiseRow)(row, totalCols)) {
        return 'TITLE';
    }
    const density = nonEmptyCount / totalCols;
...
// ── 4. 행 필터링 로직 ───────────────────────────────────────────────────
const filterRows = (rawRows, absoluteDataStartRow = -1, absoluteHeaderStartRow = -1) => {
    const totalCols = rawRows.length > 0
        ? Math.max(1, ...rawRows.slice(0, Math.min(5, rawRows.length)).map((r) => (0, exports.rowToValues)(r).length))
        : 1;
    return rawRows.filter((row, idx) => {
        ...
        if (absoluteDataStartRow !== -1 && originalRowIndex < absoluteDataStartRow) {
            ...
            if (isBeforeHeader || isHeaderUnset) {
                if ((0, exports.isApprovalLineRow)(row, idx, rawRows, totalCols) || (0, exports.isGeneralNoiseRow)(row, totalCols)) {
                    return false;
                }
            }
        }
...
const detectDataArea = (rows) => {
    ...
    while (scanStart < SCAN_LIMIT) {
        const category = (0, exports.classifyRow)(rows[scanStart], scanStart, rows, totalCols);
    ...
    for (let i = startScanIdx; i < SCAN_LIMIT - 1; i++) {
        ...
        if (i < 3 && hasNonStringOnlyRow && (0, exports.isStringOnlyRow)(sigs[i])) {
            continue;
        }
    ...
    while (headerStartRow > 0) {
        ...
        const prevValues = (0, exports.rowToValues)(prevRow).map((v) => String(v ?? '').trim());
        const nonEmptyCount = prevValues.filter((v) => v !== '').length;
        // 상위 행이 완전히 비어있거나, 결제선이거나, 문자열 전용이 아니거나, 유효 셀이 1개 이하(제목 등)이면 탐색 중단
        if (nonEmptyCount <= 1 ||
            (0, exports.isApprovalLineRow)(prevRow, headerStartRow - 1, rows, totalCols) ||
            !(0, exports.isStringOnlyRow)(sigs[headerStartRow - 1])) {
            break;
        }
        headerStartRow--;
    }
    ...
    for (let i = 0; i < SCAN_LIMIT; i++) {
        const category = (0, exports.classifyRow)(rows[i], i, rows, totalCols);
    ...
    if (firstHighDensityRowIdx !== -1) {
        if (headerStartRow < firstHighDensityRowIdx) {
            ...
            while (newHeaderStart > 0) {
                ...
                const nonEmptyCount = prevValues.filter((v) => v !== '').length;
                if (nonEmptyCount <= 1 ||
                    (0, exports.isApprovalLineRow)(prevRow, newHeaderStart - 1, rows, totalCols) ||
                    !(0, exports.isStringOnlyRow)(sigs[newHeaderStart - 1])) {
                    break;
                }
                newHeaderStart--;
            }
